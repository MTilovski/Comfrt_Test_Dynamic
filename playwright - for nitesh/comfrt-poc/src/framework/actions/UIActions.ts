...
export default class UIActions {
  ...
  //Validate Footers on Page

  public async verifyFooters(locator: string, linkName: string, description: string) {
    await test.step(`Validate footer on Page ${description}`, async () => {
      await expect(this.page.locator(locator).getByRole('link', { name: linkName, exact: true })).toBeVisible();
    });
  }
