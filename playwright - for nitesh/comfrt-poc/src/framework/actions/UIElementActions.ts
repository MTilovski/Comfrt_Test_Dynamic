...
export default class UIElementActions {
  ...
  //Close Pop up
  public async closePopUp(id: string, name: string) {
    await test.step(`Close pop-up on Home Page`, async () => {
      await this.page.locator(id).getByRole('button', { name: name }).click();
    });
  }
